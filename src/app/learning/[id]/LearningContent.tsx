...
export default function LearningContent() {
    ...
    const handleProgress = async (seconds: number, total: number) => {
        const p = (seconds / total) * 100;
        // 99.9%以上なら100%として表示、それ以外は切り捨て
        setProgress(p >= 99.9 ? 100 : Math.floor(p));
        
        // バックグラウンドで常に時刻を記録
        // 99.9%以上を完了判定のトリガーとする
        if (seconds > 0) {
            await PersistenceService.saveProgress(id, seconds, total, isCompleted || p >= 99.9);
        }

        if (p >= 99.9 && !isCompleted) {
            setIsCompleted(true);
        }
    };
    ...
    return (
        <div className="h-screen bg-slate-50 flex flex-col overflow-hidden">
            ...
            <div className="flex-1 flex flex-col lg:flex-row gap-8 p-4 sm:p-8 max-w-[1600px] mx-auto w-full overflow-hidden">
                {/* Left: Video Area */}
                <div className="flex-[2] flex flex-col space-y-6 overflow-y-auto no-scrollbar pb-6">
                    {isLoaded ? (
                        <VideoPlayer
                            videoId={videoId}
                    ...
                    <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
                        <div className="flex items-center justify-between mb-4">
                            ...
                            <div className="text-right">
                                <span className="text-2xl font-black text-indigo-600">{isCompleted ? 100 : progress}%</span>
                    ...
                    <div className="bg-white p-8 rounded-3xl border border-slate-200 shadow-xl overflow-hidden relative">
                        ...
                        <div className="space-y-8 relative">
                            ...
                            <div className="flex gap-4 relative">
                                <div className={`z-10 w-10 h-10 rounded-full flex items-center justify-center font-bold ${isCompleted ? 'bg-green-500 text-white' : 'bg-indigo-600 text-white'
                                    }`}>
                                    {isCompleted ? <CheckCircle2 className="w-6 h-6" /> : '1'}
                                </div>
                                <div>
                                    <h4 className="font-bold text-slate-800">動画講義の視聴</h4>
                                    <p className="text-sm text-slate-500">
                                        {isCompleted ? '受講完了' : `${progress}% 完了`}
                                    </p>
                                </div>
                                {/* Connector */}
                            ...
                            <div className="flex gap-4">
                                <div className={`z-10 w-10 h-10 rounded-full flex items-center justify-center font-bold ${isCompleted ? 'bg-indigo-600 text-white' : 'bg-slate-100 text-slate-400'
                                    }`}>
                                    <Award className="w-6 h-6" />
                                </div>
                                <div>
                                    <h4 className={`font-bold ${isCompleted ? 'text-slate-800' : 'text-slate-400'}`}>
                                        項目修了
                                    </h4>
                                    <p className="text-sm text-slate-400">{isCompleted ? '今月の受講は完了しています' : '100%視聴で自動記録'}</p>
                                </div>
                            </div>
                        </div>
